...
class MinecraftServerGUI:
    ...
    def open_plugin_window(self):
        ...
        self.plugin_window.title("EXE 插件管理")
        self.plugin_window.geometry("800x500")
        ...
        ttk.Button(toolbar, text="🔄 刷新", command=self.refresh_plugin_list_window, width=10).pack(side=tk.RIGHT, padx=5)
        ...
        self.create_plugin_table_window(table_frame)

        tip = ttk.Label(main, text="💡 提示：支持 .exe 格式，双击运行，右键管理", foreground=self.colors['info'])
        tip.pack(pady=(5,0))

        self.refresh_plugin_list_window()
    ...
    def create_plugin_table_window(self, parent):
        columns = ("文件名", "大小", "修改时间", "操作")
        self.exe_plugin_tree = ttk.Treeview(parent, columns=columns, show="tree headings", height=15)
        vsb = ttk.Scrollbar(parent, orient=tk.VERTICAL, command=self.exe_plugin_tree.yview)
        hsb = ttk.Scrollbar(parent, orient=tk.HORIZONTAL, command=self.exe_plugin_tree.xview)
        self.exe_plugin_tree.configure(yscrollcommand=vsb.set, xscrollcommand=hsb.set)

        self.exe_plugin_tree.heading("#0", text="⚡", anchor=tk.W)
        self.exe_plugin_tree.heading("文件名", text="文件名", anchor=tk.W)
        self.exe_plugin_tree.heading("大小", text="大小", anchor=tk.CENTER)
        self.exe_plugin_tree.heading("修改时间", text="修改时间", anchor=tk.W)
        self.exe_plugin_tree.heading("操作", text="操作", anchor=tk.CENTER)

        self.exe_plugin_tree.column("#0", width=40, minwidth=40, anchor=tk.CENTER)
        self.exe_plugin_tree.column("文件名", width=350, minwidth=200, anchor=tk.W)
        self.exe_plugin_tree.column("大小", width=80, minwidth=80, anchor=tk.CENTER)
        self.exe_plugin_tree.column("修改时间", width=150, minwidth=150, anchor=tk.W)
        self.exe_plugin_tree.column("操作", width=100, minwidth=100, anchor=tk.CENTER)

        self.exe_plugin_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        ...
        self.exe_plugin_tree.bind("<Button-3>", self.show_plugin_context_menu)
        self.exe_plugin_tree.bind("<Double-1>", lambda e: self.run_selected_plugin())

    def refresh_plugin_list_window(self):
        if self.exe_plugin_tree is None:
            return
        for item in self.exe_plugin_tree.get_children():
            self.exe_plugin_tree.delete(item)
        if not self.plugins_dir.exists():
            self.plugins_dir.mkdir(parents=True, exist_ok=True)
        exe_files = list(self.plugins_dir.glob("*.exe"))
        if not exe_files:
            self.exe_plugin_tree.insert("", tk.END, text="📁", values=("暂无 EXE 插件", "", "", ""))
            return
        for exe in sorted(exe_files):
            size = self.get_file_size(exe)
            mtime = datetime.fromtimestamp(exe.stat().st_mtime).strftime("%Y-%m-%d %H:%M:%S")
            self.exe_plugin_tree.insert("", tk.END, text="⚡", values=(exe.name, size, mtime, "▶ 运行"))
    ...
    def install_plugin(self):
        fn = filedialog.askopenfilename(title="选择可执行程序", filetypes=[("可执行文件", "*.exe"), ("所有文件", "*.*")])
        if not fn:
            return
        src = Path(fn)
        if src.suffix.lower() != '.exe':
            messagebox.showwarning("警告", "请选择 .exe 文件")
            return
        if not self.plugins_dir.exists():
            self.plugins_dir.mkdir(parents=True, exist_ok=True)
    ...
    def delete_selected_plugin(self):
        sel = self.exe_plugin_tree.selection()
        if not sel:
            messagebox.showwarning("提示", "请先选择一个插件")
            return
        values = self.exe_plugin_tree.item(sel[0], "values")
        if not values or values[0] == "暂无 EXE 插件":
            return
        name = values[0]
    ...
    def run_selected_plugin(self):
        ...
        if not path.exists():
            ...
            return
        try:
            self.log_message(f"运行插件: {name}", "info")
            if sys.platform == "win32":
    ...
    def open_selected_plugin_location(self):
        ...
        if sel:
            values = self.exe_plugin_tree.item(sel[0], "values")
            if values and values[0] != "暂无 EXE 插件":
    ...
    def show_plugin_context_menu(self, event):
        item = self.exe_plugin_tree.identify_row(event.y)
        if item:
            self.exe_plugin_tree.selection_set(item)
            vals = self.exe_plugin_tree.item(item, "values")
            if vals and vals[0] != "暂无 EXE 插件":
