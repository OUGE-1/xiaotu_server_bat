...
class MinecraftServerGUI:
    ...
    def open_plugin_window(self):
        ...
        self.plugin_status_label = ttk.Label(main, text="", foreground=self.colors['info'])
        self.plugin_status_label.pack(fill=tk.X, pady=(2,0))
    ...
    def run_selected_plugin(self):
        ...
        try:
            ...
            if self.plugin_status_label:
                self.plugin_status_label.config(text=f"✅ 已启动: {name}")
                self.plugin_window.after(3000, lambda: self.plugin_status_label.config(text=""))
        except Exception as e:
